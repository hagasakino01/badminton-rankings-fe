...
export default function DashboardPage() {
  ...
  useEffect(() => {
    ...
    async function bootstrap() {
      try {
        ...
        if (!isMounted) return;
        setStatus(error instanceof Error ? error.message : "Không thể tải danh sách bảng.");
      } finally {
        if (isMounted) {
          setLoading(false);
        }
  ...
  async function handleCreateGroup(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setStatus("Đã tạo bảng mới.");
    } catch (error) {
      setStatus(error instanceof Error ? error.message : "Không thể tạo bảng.");
    } finally {
      setSubmitting(false);
    }
  }
  ...
  async function handleDeleteGroup(groupId: string, groupName: string) {
    ...
    const confirmed = window.confirm(`Xóa bảng "${groupName}" và toàn bộ dữ liệu liên quan?`);
    if (!confirmed) return;
  ...
  const totalPlayers = groups.reduce((sum, group) => sum + (group.playerCount ?? 0), 0);
  const totalSeasons = groups.reduce((sum, group) => sum + (group.seasonCount ?? 0), 0);
  ...
  return (
    <main className="mx-auto flex w-full max-w-7xl flex-col gap-8 px-4 py-8 sm:px-6 lg:px-8 lg:py-10">
      <section className="grid gap-6 lg:grid-cols-[minmax(0,1.45fr)_minmax(320px,0.75fr)]">
        <Card className="border-0 bg-[linear-gradient(135deg,rgba(15,118,110,0.94),rgba(26,53,88,0.92))] text-primary-foreground shadow-2xl shadow-primary/15 ring-1 ring-white/10">
          <CardContent className="grid gap-8 px-6 py-8 sm:px-8 lg:px-10 lg:py-10">
            <div className="flex flex-wrap items-center gap-3">
              <Badge className="bg-white/12 text-white hover:bg-white/18">Dashboard</Badge>
              <Badge className="bg-white/10 text-white/75 hover:bg-white/15">Season management</Badge>
            </div>

            <div className="space-y-4">
              <h1 className="max-w-3xl font-heading text-4xl leading-none font-semibold tracking-tight text-balance sm:text-5xl">
                Điều phối các bảng đấu và mùa giải từ một mặt điều khiển duy nhất.
              </h1>
              <p className="max-w-2xl text-base leading-7 text-white/78">
                Tạo group mới, theo dõi quy mô câu lạc bộ và đi thẳng vào mùa giải đang cần thao tác
                mà không phải rời khỏi dashboard.
              </p>
            </div>

            <div className="grid gap-3 sm:grid-cols-3">
              <div className="rounded-3xl bg-black/12 p-5">
                <p className="text-xs uppercase tracking-[0.24em] text-white/55">Bảng đấu</p>
                <p className="mt-3 font-heading text-4xl font-semibold">{groups.length}</p>
              </div>
              <div className="rounded-3xl bg-black/12 p-5">
                <p className="text-xs uppercase tracking-[0.24em] text-white/55">Tổng VĐV</p>
                <p className="mt-3 font-heading text-4xl font-semibold">{totalPlayers}</p>
              </div>
              <div className="rounded-3xl bg-black/12 p-5">
                <p className="text-xs uppercase tracking-[0.24em] text-white/55">Mùa giải</p>
                <p className="mt-3 font-heading text-4xl font-semibold">{totalSeasons}</p>
              </div>
            </div>
          </CardContent>
        </Card>
        ...
        <Card className="border-0 bg-card/90 shadow-xl shadow-black/5 ring-1 ring-black/6">
          <CardHeader className="space-y-3">
            <CardTitle className="font-heading text-2xl">Phiên làm việc</CardTitle>
            <CardDescription>Thoát tài khoản hiện tại hoặc tiếp tục mở rộng cấu trúc giải đấu.</CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            <div className="rounded-3xl border border-border/70 bg-muted/40 p-4 text-sm leading-6 text-muted-foreground">
              Dashboard này là điểm vào chính để tạo bảng, mở mùa, sinh session và đi sâu vào quản lý
              từng nhóm người chơi.
            </div>
            <Button variant="outline" size="lg" className="w-full justify-center gap-2" onClick={handleLogout}>
      ...
      {status && (
        <Alert>
          <AlertTitle>Trạng thái hệ thống</AlertTitle>
        ...
        <Card className="border-0 bg-card/90 shadow-lg shadow-black/5 ring-1 ring-black/6">
          <CardHeader className="space-y-3">
            ...
            <div className="space-y-2">
              <CardTitle className="font-heading text-2xl">Tạo bảng đấu mới</CardTitle>
              <CardDescription>
                Mỗi bảng là một không gian riêng để quản lý roster, mùa giải và toàn bộ lịch sử thi đấu.
              </CardDescription>
            </div>
          </CardHeader>
          <CardContent>
            <form className="space-y-5" onSubmit={handleCreateGroup}>
              ...
              <Button type="submit" size="lg" disabled={submitting} className="w-full justify-center gap-2">
        ...
        <div className="grid gap-6">
          ...
          <Card className="border-0 bg-card/90 shadow-lg shadow-black/5 ring-1 ring-black/6">
            <CardHeader className="space-y-2">
              <CardTitle className="font-heading text-2xl">Các bảng bạn đang quản lý</CardTitle>
              <CardDescription>
                Chọn một bảng để vào roster, season, session và bảng xếp hạng của riêng nhóm đó.
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              {loading ? (
                      ...
                      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                        <div className="space-y-2">
                          ...
                          <CardDescription className="max-w-2xl text-sm leading-6">
                            {group.description || "Chưa có mô tả cho bảng đấu này."}
                          </CardDescription>
                        </div>
                        <Badge variant="secondary">{group.playerCount ?? 0} người chơi</Badge>
                      </div>
                    </CardHeader>
                    <CardContent className="grid gap-4 sm:grid-cols-[minmax(0,1fr)_auto] sm:items-center">
                      <div className="flex flex-wrap gap-2 text-sm text-muted-foreground">
                        <span className="rounded-full bg-muted px-3 py-1">Mùa giải: {group.seasonCount ?? 0}</span>
                        <span className="rounded-full bg-muted px-3 py-1">Owner: bạn</span>
                      </div>
                      <div className="flex flex-wrap items-center gap-2">
                        ...
                        >
                          {deletingGroupId === group._id ? "Đang xóa..." : "Xóa bảng"}
                        </Button>
                      </div>
                    </CardContent>
                  </Card>
                ...
                <div className="rounded-3xl border border-dashed border-border bg-muted/30 px-5 py-10 text-center">
                  ...
                  <p className="mt-1 text-sm text-muted-foreground">
                    Hãy tạo bảng đầu tiên để bắt đầu quản lý người chơi và mùa giải.
                  </p>
                </div>
              )}
            </CardContent>
